const ControllerModel = require('../models/release-notes.model');
...
module.exports = {
    ...
    getLatest: async (req, res, next) => {
        res.send(await ControllerModel.findOne({}, {}, { sort: { 'createdAt': -1 } }));
    },
    ...
    add: (req, res, next) => {
        if (!req.body.role || !req.body.role._id) {
            try { req.body.role = sharedController.ACL_DATA.roles[1] } catch (err) { console.warn(`Not able to assign default role: user`) }
        }
    ...
    findOneAndUpdate: async (req, res, next) => {
        ...
        if (user) {
            ...
            return res.send(await ControllerModel.find());
        } else {
            res.status(500).send({
